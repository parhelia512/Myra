namespace Myra.Graphics2D.UI.Styles
{
	/// <summary>
	/// Style class for list box controls.
	/// </summary>
	public class ListBoxStyle: WidgetStyle
	{
		/// <summary>
		/// Gets or sets the style for list items.
		/// </summary>
		public ImageTextButtonStyle ListItemStyle { get; set; }

		/// <summary>
		/// Gets or sets the style for item separators.
		/// </summary>
		public SeparatorStyle SeparatorStyle { get; set; }

		/// <summary>
		/// Initializes a new instance of the ListBoxStyle class.
		/// </summary>
		public ListBoxStyle()
		{
		}

		/// <summary>
		/// Initializes a new instance of the ListBoxStyle class by copying properties from another style.
		/// </summary>
		/// <param name="style">The list box style to copy from.</param>
		public ListBoxStyle(ListBoxStyle style) : base(style)
		{
			ListItemStyle = style.ListItemStyle != null ? new ImageTextButtonStyle(style.ListItemStyle) : null;
			SeparatorStyle = style.SeparatorStyle != null ? new SeparatorStyle(style.SeparatorStyle) : null;
		}

		/// <summary>
		/// Creates a copy of this list box style.
		/// </summary>
		/// <returns>A new list box style with the same properties.</returns>
		public override WidgetStyle Clone()
		{
			return new ListBoxStyle(this);
		}
	}
}