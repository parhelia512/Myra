namespace Myra.Graphics2D.UI.Styles
{
	/// <summary>
	/// Style class for slider controls.
	/// </summary>
	public class SliderStyle: WidgetStyle
	{
		/// <summary>
		/// Gets or sets the style for the slider's draggable knob.
		/// </summary>
		public ImageButtonStyle KnobStyle { get; set; }

		/// <summary>
		/// Initializes a new instance of the SliderStyle class.
		/// </summary>
		public SliderStyle()
		{
		}

		/// <summary>
		/// Initializes a new instance of the SliderStyle class by copying properties from another style.
		/// </summary>
		/// <param name="style">The slider style to copy from.</param>
		public SliderStyle(SliderStyle style) : base(style)
		{
			KnobStyle = style.KnobStyle != null ? new ImageButtonStyle(style.KnobStyle) : null;
		}

		/// <summary>
		/// Creates a copy of this slider style.
		/// </summary>
		/// <returns>A new slider style with the same properties.</returns>
		public override WidgetStyle Clone()
		{
			return new SliderStyle(this);
		}
	}
}
