/* Generated by MyraPad at 12/27/2023 7:14:06 PM */
using Myra;
using Myra.Graphics2D;
using Myra.Graphics2D.TextureAtlases;
using Myra.Graphics2D.UI;
using Myra.Graphics2D.Brushes;
using Myra.Graphics2D.UI.Properties;
using FontStashSharp.RichText;
using AssetManagementBase;

#if STRIDE
using Stride.Core.Mathematics;
#elif PLATFORM_AGNOSTIC
using System.Drawing;
using System.Numerics;
using Color = FontStashSharp.FSColor;
#else
// MonoGame/FNA
using Microsoft.Xna.Framework;
using Microsoft.Xna.Framework.Graphics;
#endif

namespace Myra.Graphics2D.UI.File
{
	partial class FileDialog : Dialog
	{
		private void BuildUI()
		{
			var image1 = new Image();

			_buttonBack = new Button();
			_buttonBack.HorizontalAlignment = Myra.Graphics2D.UI.HorizontalAlignment.Center;
			_buttonBack.VerticalAlignment = Myra.Graphics2D.UI.VerticalAlignment.Center;
			_buttonBack.Id = "_buttonBack";
			_buttonBack.Content = image1;

			var image2 = new Image();

			_buttonForward = new Button();
			_buttonForward.HorizontalAlignment = Myra.Graphics2D.UI.HorizontalAlignment.Center;
			_buttonForward.VerticalAlignment = Myra.Graphics2D.UI.VerticalAlignment.Center;
			_buttonForward.Id = "_buttonForward";
			_buttonForward.Content = image2;

			_textFieldPath = new TextBox();
			_textFieldPath.Readonly = true;
			_textFieldPath.VerticalAlignment = Myra.Graphics2D.UI.VerticalAlignment.Center;
			_textFieldPath.Id = "_textFieldPath";
			StackPanel.SetProportionType(_textFieldPath, Myra.Graphics2D.UI.ProportionType.Fill);

			var image3 = new Image();

			_buttonParent = new Button();
			_buttonParent.HorizontalAlignment = Myra.Graphics2D.UI.HorizontalAlignment.Center;
			_buttonParent.VerticalAlignment = Myra.Graphics2D.UI.VerticalAlignment.Center;
			_buttonParent.Id = "_buttonParent";
			_buttonParent.Content = image3;

			var horizontalStackPanel1 = new HorizontalStackPanel();
			horizontalStackPanel1.Spacing = 4;
			horizontalStackPanel1.Widgets.Add(_buttonBack);
			horizontalStackPanel1.Widgets.Add(_buttonForward);
			horizontalStackPanel1.Widgets.Add(_textFieldPath);
			horizontalStackPanel1.Widgets.Add(_buttonParent);

			_listPlaces = new ListView();
			_listPlaces.HorizontalAlignment = Myra.Graphics2D.UI.HorizontalAlignment.Stretch;
			_listPlaces.ClipToBounds = true;
			_listPlaces.Id = "_listPlaces";

			_gridFiles = new Grid();
			_gridFiles.ColumnSpacing = 4;
			_gridFiles.RowSpacing = 4;
			_gridFiles.ColumnsProportions.Add(new Proportion
			{
				Type = Myra.Graphics2D.UI.ProportionType.Auto,
			});
			_gridFiles.ColumnsProportions.Add(new Proportion
			{
				Type = Myra.Graphics2D.UI.ProportionType.Fill,
			});
			_gridFiles.GridSelectionMode = Myra.Graphics2D.UI.GridSelectionMode.Row;
			_gridFiles.Id = "_gridFiles";

			_scrollPane = new ScrollViewer();
			_scrollPane.ShowHorizontalScrollBar = false;
			_scrollPane.Id = "_scrollPane";
			_scrollPane.Content = _gridFiles;

			_splitPane = new HorizontalSplitPane();
			_splitPane.Id = "_splitPane";
			StackPanel.SetProportionType(_splitPane, Myra.Graphics2D.UI.ProportionType.Fill);
			_splitPane.Widgets.Add(_listPlaces);
			_splitPane.Widgets.Add(_scrollPane);

			_textBlockFileName = new Label();
			_textBlockFileName.Text = "File name";
			_textBlockFileName.Id = "_textBlockFileName";

			_textFieldFileName = new TextBox();
			_textFieldFileName.Readonly = true;
			_textFieldFileName.Id = "_textFieldFileName";
			StackPanel.SetProportionType(_textFieldFileName, Myra.Graphics2D.UI.ProportionType.Fill);

			var horizontalStackPanel2 = new HorizontalStackPanel();
			horizontalStackPanel2.Spacing = 4;
			horizontalStackPanel2.Widgets.Add(_textBlockFileName);
			horizontalStackPanel2.Widgets.Add(_textFieldFileName);

			var verticalStackPanel1 = new VerticalStackPanel();
			verticalStackPanel1.Spacing = 4;
			verticalStackPanel1.Widgets.Add(horizontalStackPanel1);
			verticalStackPanel1.Widgets.Add(_splitPane);
			verticalStackPanel1.Widgets.Add(horizontalStackPanel2);


			Title = "Open File...";
			Left = 658;
			Top = 131;
			Width = 600;
			Height = 400;
			Content = verticalStackPanel1;
		}

		/// <summary>
		/// The back navigation button.
		/// </summary>
		public Button _buttonBack;

		/// <summary>
		/// The forward navigation button.
		/// </summary>
		public Button _buttonForward;

		/// <summary>
		/// The text field displaying the current folder path.
		/// </summary>
		public TextBox _textFieldPath;

		/// <summary>
		/// The button to navigate to the parent folder.
		/// </summary>
		public Button _buttonParent;

		/// <summary>
		/// The list view displaying available places (drives and folders).
		/// </summary>
		public ListView _listPlaces;

		/// <summary>
		/// The grid displaying files and folders in the current folder.
		/// </summary>
		public Grid _gridFiles;

		/// <summary>
		/// The scroll viewer for the file grid.
		/// </summary>
		public ScrollViewer _scrollPane;

		/// <summary>
		/// The split pane dividing the places list and file grid.
		/// </summary>
		public HorizontalSplitPane _splitPane;

		/// <summary>
		/// The label for the file name input field.
		/// </summary>
		public Label _textBlockFileName;

		/// <summary>
		/// The text field for entering the file name.
		/// </summary>
		public TextBox _textFieldFileName;
	}
}
